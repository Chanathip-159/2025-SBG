...
# Edit zone
############################################
$dest_file="department";																																			// destination name of file
...
if($_POST['add_edit_sender_id']) {
	$sql = "UPDATE SBG.dbo.DEPARTMENT
					SET DEPARTMENT_Name='".$_POST['DEPARTMENT_Name']."'
					,DEPARTMENT_Description='".$_POST['DEPARTMENT_Description']."'
					WHERE DEPARTMENT_Name ='".$_POST['add_edit_sender_id']."'
					AND DEPARTMENT_Username ='".$_SESSION[_SES_USR_NAME]."'
					";
					// echo $sql;
	...
	if (querySqlEx($sql) == false) {
		...
		$_GET['toast_message']="Fail to update DEPARTMENT";
	} else {
		$_GET['toast_type']="S";
		$_GET['toast_header']="Result message";
		$_GET['toast_message']="Update DEPARTMENT successfully";
	}
}

if($_POST['del_item']) {
	$sql = "DELETE FROM SBG.dbo.DEPARTMENT WHERE DEPARTMENT_Name ='".$_POST['del_item']."' ";
	...
	if (querySqlEx($sql) == false) {
		...
		$_GET['toast_type']="F";
		$_GET['toast_header']="Fail to process";
		$_GET['toast_message']="Fail to Delete Department";
	} else {
		$_GET['toast_type']="S";
		$_GET['toast_header']="Result message";
		$_GET['toast_message']="Delete Department successfully";
	}
}


if($_POST['submit_add_DEPARTMENT']) {
	...
	$sql_chk_DEPARTMENT = "SELECT count(DEPARTMENT_Name) as count FROM SBG.dbo.DEPARTMENT WHERE DEPARTMENT_Name = '".$DEPARTMENT."' ";

	$count_DEPARTMENT = querySqlSingleRowEx($sql_chk_DEPARTMENT);

	if($count_DEPARTMENT['count'] > 0) {
		$_GET['toast_type']="F";
		$_GET['toast_header']="Fail to process";
		$_GET['toast_message']="This Department already exists.";
	} else {
		$sql = "INSERT INTO SBG.dbo.DEPARTMENT (DEPARTMENT_Name, DEPARTMENT_Description, DEPARTMENT_CreateDt, DEPARTMENT_UpdateDt, DEPARTMENT_Username)
						VALUES ('".$DEPARTMENT."', '".$desc."', '".$create_dt."', '".$update_dt."', '".$_SESSION[_SES_USR_NAME]."')
					";
		// echo $sql_insert;
		...
		if (querySqlEx($sql) == false) {
			...
			$_GET['toast_type']="F";
			$_GET['toast_header']="Fail to process";
			$_GET['toast_message']="Fail to upload Department";
		} else {
			$_GET['toast_type']="S";
			$_GET['toast_header']="Result message";
			$_GET['toast_message']="Upload Department successfully.";
		}
	}
}

$search_bar = "element/_e_search_bar.php"; // "_e_search_bar.php"
...
if($_SESSION[_SES_USR_NAME] != 'admin') { // user
	$ico_conditions = "IIF(DEPARTMENT_Username = '".$_SESSION[_SES_USR_NAME]."', '_ICON_1_', '*Not allow to edit') AS icon1";
} else { // admin 
	$ico_conditions = "IIF(DEPARTMENT_Username != '', '_ICON_1_', '*Not allow to edit') AS icon1";
}
...
$fields_info=Array(
	 Array(_SQL_FIELD=>"DEPARTMENT_Name" // can use [ISAG].[dbo].[Users].[User_CreateDate] // can use [ISAG].[dbo].[Users].[User_Name] AS Name
		,_LAB_FIELD=>"Name"
		,_TYP_FIELD=>1
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"DEPARTMENT_Description"
		,_LAB_FIELD=>"Description"
		,_TYP_FIELD=>1)
	,Array(_SQL_FIELD=>"DEPARTMENT_CreateDt"
		,_LAB_FIELD=>"Create DT"
		,_TYP_FIELD=>1
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"DEPARTMENT_UpdateDt"
	...
	,Array(_SQL_FIELD=>$ico_conditions
		,_LAB_FIELD=>"Action"
		,_TYP_FIELD=>100
		,_RID_FIELD=>'DEPARTMENT_Name' // field id to put in _SID_1_ to _SID_3_ format Service_ID,Provider_ID
...
############################################
?>
	<h1>Departments Management</h1>	<hr>
	<h5><i class="fa fa-cog"></i>&nbsp;Add new department</h5>	<br>
	<form enctype="multipart/form-data" role="form" name="form_add_sender" action="<?=_MAIN_DF;?>department" method="POST">
		<div class="form-row align-items-center">
			<div class="form-group">
				<input type="text" id="DEPARTMENT" name="DEPARTMENT" value="" class="form-control" placeholder="Department name" size="20" required>
			</div>&nbsp;
			<div class="form-group">
				<input type="text" id="desc" name="desc" value="" class="form-control" placeholder="Description" size="30">
			</div>&nbsp;
			<div class="form-group">
			<input type="submit" id="submit_add_DEPARTMENT" name="submit_add_DEPARTMENT" class="btn btn-primary text-white" value="submit">
			</div>
		</div>
	</form>
	<hr>
...
############################################
	...
	require_once("element/_p_dialog_add_edit_dp.php");
	// require_once("_t_DEPARTMENT_list.php");
}
?>
	<hr>
