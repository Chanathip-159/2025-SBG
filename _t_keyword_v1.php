...
if($_POST['submit_add_sender'])
{
	...
	for($i=0; $i<$j; $i++)
	{
		$sql_chk_user = "SELECT count(CUSTOMER_Username) as count
										 ...
										 WHERE CUSTOMER_Username = '".$array_sql[$i][1]."' 
										";

		$sql_chk_user_sender = "SELECT KEYWORD_Name as key_name, KEYWORD_Username as username
														FROM SBG.dbo.KEYWORD 
														WHERE KEYWORD_Name = '".$array_sql[$i][0]."' 
														AND KEYWORD_Username = '".$array_sql[$i][1]."';
													 ";
		...
		while ($row_chk_use = $stmt_chk_user->fetch(PDO::FETCH_ASSOC))
		{
			...
			if($row_chk_use['count'] > 0)
			{
				if( $stmt_chk_user_sender->fetch(PDO::FETCH_ASSOC) )
				{
					...
					if($row_chk_user_sender['key_name'] = $array_sql[$i][0] && $row_chk_user_sender['username'] = $array_sql[$i][1])
					{
						$msg_err2 = 'ไม่สามารถเพิ่ม keyword ได้ เนื่องจากมีข้อมูลอยู่แล้ว';
						$str_sql_err2 .= "(
							'".$array_sql[$i][0]."',
							'".$array_sql[$i][1]."'
						),";

						$_SESSION['str_err2_msg'] .= $array_sql[$i][0].'|'.$array_sql[$i][1].'<br>';
					}
				} else {
					$msg_result = 'เพิ่มได้';
					$str_sql .= "(
				...
				$msg_err1 = 'ไม่สามารถเพิ่ม keyword ได้ เนื่องจากไม่มี username อยู่ในระบบ ';
				$str_sql_err1 .= "(
					'".$array_sql[$i][0]."',
					'".$array_sql[$i][1]."',
					".$array_sql[$i][2].",
				...
				$_SESSION['str_err1_msg'] .= $array_sql[$i][0].'|'.$array_sql[$i][1].'<br>';
			}
		}
...
$sql_where="KEYWORD_Username = '".$_SESSION[_SES_USR_NAME]."' ";
...
?>
			...
			<hr>
			<h5><i class="fa fa-cog"></i>&nbsp;Add or upload keyword</h5>
			<br>
			...
			<img src="./img/ex_keyword.png" width='400' height='200'>
			<hr>
