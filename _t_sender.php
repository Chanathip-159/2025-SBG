...
// upload sender process
if($_POST['submit_sender_byfile'])
{
		...
		while ($row_chk_use = $stmt_chk_user->fetch(PDO::FETCH_ASSOC))
		{
			...
			if($row_chk_use['count'] > 0)
			{
				if( $stmt_chk_user_sender->fetch(PDO::FETCH_ASSOC) )
				{
					...
					if($row_chk_user_sender['sender'] = $array_sql[$i][1])
					{
...
if($_POST['submit_add_sender']) {
	...
	// $sql_chk_sender = "SELECT count(SENDER_Sender) as count_sender, count(SENDER_Username) as count_username FROM SBG.dbo.SENDERS WHERE SENDER_Sender = '".$sender_name."' AND SENDER_Username = '".$sender_username."' ";
	$sql_chk_sender = "SELECT count(SENDER_Sender) as count_sender FROM SBG.dbo.SENDERS WHERE SENDER_Sender = '".$sender_name."' ";

	$count_sender = querySqlSingleRowEx($sql_chk_sender);

	if($count_sender['count_sender'] > 0) {
		$_GET['toast_type']="F";
		$_GET['toast_header']="Fail to process";
		$_GET['toast_message']="This sender already exists.";
	} else {
		$sql = "INSERT INTO SBG.dbo.SENDERS (SENDER_Username, SENDER_Sender, SENDER_CreateDt, SENDER_UpdateDt, SENDER_Status, CATEGORY_Id, PRIORITY_flag)
						VALUES ('".$sender_username."', '".$sender_name."', '".$create_dt."', '".$update_dt."', '".$status."', '".$category."', '".$priority."')
					";
		echo $sql;
...
############################################
?>
	<h1>Sender Management</h1>	<hr>
	<!-- insert form -->
	<h5><i class="fa fa-cog"></i>&nbsp;Add new sender</h5>	<br>
	<form enctype="multipart/form-data" role="form" name="form_add_sender" action="<?=_MAIN_DF;?>sender" method="POST">
		<div class="form-row align-items-center">
			<div class="form-group">
				<input type="text" id="sender_name" name="sender_name" value="" class="form-control" placeholder="Sender name" size="20" required>
			</div>&nbsp;
			<div class="form-group">
				<input type="text" id="sender_username" name="sender_username" value="" class="form-control" placeholder="Username" size="20" required>
			</div>&nbsp;
			<div class="form-group">
			...
			<div class="form-group">
				<select name="category_id" class="form-control" required>
					...
					<option value='2'>Normal text</option>
	...
	<!-- insert form -->
	<!-- upload form -->
	<h5><i class="fa fa-cog"></i>&nbsp;Add new sender by file</h5>	<br>
	<form enctype="multipart/form-data" role="form" name="form_add_sender" action="<?=_MAIN_DF;?>sender" method="POST">
		<div class="row col-md-4 mb-3 custom-file">
			<input type="file" name="fileToUpload" id="fileToUpload" required>
		</div><br>
		<input type="submit" id="submit_sender_byfile" name="submit_sender_byfile" class="btn btn-primary text-white" value="submit">
	</form><br>
	<img src="./img/sender_pattern_20230104.png" width='450' height='300'>
	<!-- upload form -->
	<hr>
