<?php
  ...
  $sql = "SELECT CUSTOMER_Username, CUSTOMER_NeedDr, CUSTOMER_ExpireDate, CUSTOMER_AccountUsage, CUSTOMER_CurrentUsage, CUSTOMER_MonthlyUsage, CUSTOMER_Telephone FROM SBG.dbo.CUSTOMERS WHERE CUSTOMER_Telephone IS NOT NULL";
  ...
  if (($stmt = querySqlEx($sql)) == false) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      $checkDate = date_create(Date('Y-m-d', strtotime('+30 days')));
      ...
      if($accUsage == -1 || $curUsage == 0) {
        ...
        $cal = cal_percentage($curUsage, $accUsage).'%<br/>';
  ...
  foreach($arr_conditions1 as $row) {
    ...
    try {
      ...
      $msg_header = "แจ้งเตือนระบบ SBG";

  //     $msg_sms = "เรียน คุณ ".$row['username']."
  // <br />
  // ขณะนี้ Account ของท่านกำลังจะหมดอายุภายใน 30 วัน กรุณาติดต่อผู้ดูแลระบบเพื่อขยายระยะเวลาการใช้งาน
  // <br />";

      $msg_sms = "เรียนผู้ใช้งานระบบ SMS Bulk Gateway (".$row['username'].")

ขณะนี้บัญชีของคุณกำลังจะหมดอายุภายใน 30 วัน กรุณาติดต่อผู้ดูแลระบบ";
  ...
  //     $msg_sms = "เรียน คุณ ".$row['username']."
  // <br />
  // ขณะนี้ Account ของท่านเหลือโควต้าการใช้งาน sms ต่ำกว่า 5% กรุณาติดต่อผู้ดูแลระบบเพื่อขยายโควต้าการใช้งาน
  // <br />";

      $msg_sms = "เรียนผู้ใช้งานระบบ SMS Bulk Gateway (".$row['username'].")

ขณะนี้จำนวนการใช้งาน SMS เหลือต่ำกว่า 5% กรุณาติดต่อผู้ดูแลระบบ";
...
ขณะนี้จำนวนการใช้งาน SMS ประจำเดือนเหลือต่ำกว่า 5% กรุณาติดต่อผู้ดูแลระบบ";
