...
# Edit zone
############################################
$dest_file="sub_user";																																			// destination name of file
...
if (strlen($_POST['add_parent_customer_id'])) {
	...
	if (isset($user_id)) {
		...
		if ($_POST['CUSTOMER_Parent_Username'] != $_POST['CUSTOMER_Username']) 
		{

			$sql="SELECT COUNT(*) AS Counting FROM SBG.dbo.CUSTOMERS WHERE LOWER(CUSTOMER_Username)=LOWER(?)";
			if (querySqlSingleFieldEx($sql,[$user_id]))
			{
				$sql_check_sub ="SELECT COUNT(*) AS Count_sub FROM SBG.dbo.CUSTOMERS WHERE [CUSTOMER_Parent_Username]='".$_POST['CUSTOMER_Username']."'";
				...
				$sql_accUsage_parent = "SELECT CUSTOMER_AccountUsage
																...
																WHERE LOWER(CUSTOMER_Username)='".$_POST['CUSTOMER_Username']."'
																";
				...
				$sql_accUsage_sub = "SELECT SUM(CUSTOMER_AccountUsage) as CUSTOMER_AccountUsage
															FROM [SBG].[dbo].[CUSTOMERS]
															WHERE LOWER(CUSTOMER_Parent_Username)='".$_POST['CUSTOMER_Username']."'
															";
				...
				if($_SESSION['remain_usage'] >= $_POST['CUSTOMER_AccountUsage'])
				{
					...
					if (!querySqlEx($sql)) {
						...
						if($count_sub['Count_sub'] == 0)
						{
							...
							{
								$sql="UPDATE [SBG].[dbo].[CUSTOMERS]
											SET [CUSTOMER_Parent_Username]='".$_POST['CUSTOMER_Username']."'
											WHERE LOWER(CUSTOMER_Username)='".$_POST['CUSTOMER_Parent_Username']."'
											";
							...
							$sql="UPDATE [SBG].[dbo].[CUSTOMERS]
										SET [CUSTOMER_Parent_Username]='".$_POST['CUSTOMER_Username']."', [CUSTOMER_AccountUsage]='".$_POST['CUSTOMER_AccountUsage']."'
										WHERE LOWER(CUSTOMER_Username)='".$_POST['CUSTOMER_Parent_Username']."'
										";
...
// $sql_where="exists (SELECT * FROM SBG.dbo.CUSTOMERS as a2 WHERE a1.CUSTOMER_Username = a2.CUSTOMER_Parent_Username) $searchByForm";
$sql_where="CUSTOMERS.CUSTOMER_Parent_Username IS NOT NULL $searchByForm";
...
$fields_info=Array(
	 Array(_SQL_FIELD=>"CUSTOMER_Parent_Username" // can use [ISAG].[dbo].[Users].[User_CreateDate] // can use [ISAG].[dbo].[Users].[User_Name] AS Name
		,_LAB_FIELD=>"Username"
		,_TYP_FIELD=>1
		,_SORT_FIELD=>1
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"CUSTOMER_Username"
		,_LAB_FIELD=>"Sub-user"
		,_TYP_FIELD=>1
		,_SORT_FIELD=>1
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"CUSTOMER_MonthlyUsage"
		,_LAB_FIELD=>"Monthly Usage (sub-user)"
		,_TYP_FIELD=>3
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"CUSTOMER_AccountUsage"
		,_LAB_FIELD=>"Account Usage (sub-user)"
		,_TYP_FIELD=>3
		,_SEA_FIELD=>true)
	,Array(_SQL_FIELD=>"CUSTOMER_CurrentUsage"
		,_LAB_FIELD=>"Current Usage (sub-user)"
		,_TYP_FIELD=>3
		,_SEA_FIELD=>true)
...
?>
			<h1>
				Sub-user Manage
				<a href="#" data-toggle="modal" id="_ADD_" data-target="#addParentModal"><i class="fa fa-fw fa-plus-circle" style="font-size:30px"></i></a>
			</h1>
			<hr>
